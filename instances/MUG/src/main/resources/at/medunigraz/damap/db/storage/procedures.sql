CREATE OR REPLACE PROCEDURE damap_insert_storage(url_ varchar, version_ integer, location_ varchar, backup_location_ varchar)
LANGUAGE SQL
AS $$
    INSERT INTO
        internal_storage ( id, version, url, storage_location, backup_location ) 
    values
        (
            NEXTVAL ('internal_storage_seq'), version_, url_, location_, backup_location_ 
        )
$$;


CREATE OR REPLACE PROCEDURE damap_update_storage(old_url_ varchar, url_ varchar, version_ integer, location_ varchar, backup_location_ varchar)
LANGUAGE SQL
AS $$
    UPDATE
        internal_storage 
...
CREATE OR REPLACE PROCEDURE damap_upsert_storage(old_url_ varchar, url_ varchar, version_ integer, location_ varchar, backup_location_ varchar)
LANGUAGE plpgsql
AS $$
    BEGIN
        IF NOT EXISTS
        ...
        THEN
            CALL damap_insert_storage(url_, version_, location_, backup_location_);
        ELSE
            CALL damap_update_storage(old_url_, url_, version_, location_, backup_location_);
        END IF;        
    END
$$;
